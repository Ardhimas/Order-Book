...
void ordered_insert(std::vector<pricepair> &vect, Order order):
  if side == "B"
...
void print_result(std::map order_map, std::vector<pricepair> vect, char side, double &money, int &sum, int target_size)
{
  ...
  else if (sum >= target_size)
  {
    ...
    while (counter > 0)
    {
      ...
      for (std::vector<pricepair>::iterator it = vect.begin(); it!= vect.end(); ++it)
      {
        temp_price = *it.first;
